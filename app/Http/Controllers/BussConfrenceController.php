...
use App\Http\Requests\BussConfrenceRequest;
use Yajra\DataTables\Facades\DataTables;
use App\Models\BussConfrence;

class BussConfrenceController extends Controller
{
    use CommonFunctions;
    /**
     ...
     */
    public function viewConfrence()
    {
        
        return view("Dashboard.Pages.manageBussConfrence");
    }

    public function saveConfrence(BussConfrenceRequest $request)
    {
        Cache::forget("buss_confrences");
    ...
    public function multipleImage(BussConfrenceRequest $request)
    {
        $maxId = BussConfrence::max(BussConfrence::ID);
        $maxId += 1;
    ...
    public function insertData(BussConfrenceRequest $request)
    {
            $image_url = "";         
            ...
            $createNewRow = new BussConfrence();
    ...
    public function updateData(BussConfrenceRequest $request)
    {        
            $updateModel = BussConfrence::find($request->id);
    ...
    public function enableRow(BussConfrenceRequest $request)
    {
        $check = BussConfrence::where('id', $request->id)->first();
        if ($check) {
            $check->status = 1;
            $check->save();
    ...
    public function disableRow(BussConfrenceRequest $request)
    {
        $check = BussConfrence::where('id', $request->id)->first();
        if ($check) {
            $check->status = 0;
    ...
    public function confrenceData()
    {
        $query = BussConfrence::select(
    ...
    public function getConfrence()
    {
        $conference = BussConfrence::where('status', 1)->orderBy('updated_at', 'desc')->first();
        $data = [
            'status' => true,
            'success' => true,
