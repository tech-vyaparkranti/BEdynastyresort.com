...
class ActivityController extends Controller
{
    ...
    {
        return view("Dashboard.Pages.manageActivity");
    }

    public function saveActivity(ActivityRequest $request)
    {
    ...
    public function ImageUpload(ActivityRequest $request)
    {
        $maxId = Activity::max(Activity::ID);
        $maxId += 1;
    ...
    public function insertData(ActivityRequest $request)
    {
            $image_url = "";
    ...
    public function updateData(ActivityRequest $request)
    {
            $updateModel = Activity::find($request->id);
    ...
    public function enableRow(ActivityRequest $request)
    {
        $check = Activity::where('id', $request->id)->first();
        if ($check) {
            $check->status = 1;
            $check->save();
    ...
    public function disableRow(ActivityRequest $request)
    {
        $check = Activity::where('id', $request->id)->first();
        if ($check) {
            $check->status = 0;
